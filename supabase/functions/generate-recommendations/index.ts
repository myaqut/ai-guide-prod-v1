...
const INFERRABLE_FIELDS = new Set([
  ...
  'sid', // Can be generated from product name (remove spaces)
]);
...
function canInferWithoutSearch(fieldName: string, workflowType: WorkflowType): boolean {
  ...
  // SID for applications (product name without spaces)
  if (workflowType === 'application' && lowerName === 'sid') return true;
...
function generateInferredRecommendation(
  ...
  // SID inference for applications (product name without spaces)
  if (workflowType === 'application' && lowerName === 'sid') {
    const sid = componentName.replace(/\s+/g, '');
    return {
      recommendation: sid,
      confidence: 1.0,
      reasoning: `SID generated by removing spaces from the application name "${componentName}".`
    };
  }
...
- SID: Product name with spaces removed (e.g., "SalesforceSalesCloud", "MicrosoftTeams")
