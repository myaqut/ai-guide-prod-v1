...
const INFERRABLE_FIELDS = new Set([
  ...
  'siid', 'si id', // Can be generated from product name (remove spaces)
]);
...
function canInferWithoutSearch(fieldName: string, workflowType: WorkflowType): boolean {
  ...
  // SI ID for applications (product name without spaces)
  if (workflowType === 'application' && (lowerName === 'siid' || lowerName === 'siid')) return true;
...
function generateInferredRecommendation(
  ...
  // SI ID inference for applications (product name without spaces)
  if (workflowType === 'application' && (lowerName === 'siid' || lowerName === 'siid')) {
    const siId = componentName.replace(/\s+/g, '');
    return {
      recommendation: siId,
      confidence: 1.0,
      reasoning: `SI ID generated by removing spaces from the application name "${componentName}".`
    };
  }
...
function isApplicationSearchField(fieldName: string): boolean {
  const searchableFields = [
    'description', 'webpage', 'website', 'product category', 'category', 'pricing', 
    'hosting', 'sso', 'internal comment', 'g2'
  ];
...
function buildApplicationSearchQuery(appName: string, fieldName: string, vendorDomain?: string, enforceOfficialDomain?: boolean): string {
  ...
  // Product Category - G2 category
  if (lowerFieldName.includes('product category') || lowerFieldName.includes('category')) {
    // Always search G2 for this field, ignore domain restriction
    return `site:g2.com "${appName}" category software reviews`;
  }
  
  // Pricing Type - summarize pricing
  if (lowerFieldName.includes('pricing type') || (lowerFieldName.includes('pricing') && !lowerFieldName.includes('url'))) {
    return `${domainFilter} "${appName}" pricing plans cost subscription free trial enterprise ${!enforceOfficialDomain ? officialSourceHint : ''}`.trim();
  }
  
  // Pricing URL - official pricing page
  if (lowerFieldName.includes('pricing') && lowerFieldName.includes('url')) {
    return `${domainFilter} "${appName}" pricing page plans cost ${!enforceOfficialDomain ? officialSourceHint : ''}`.trim();
  }
  
  // Hosting Description - specifically look for AWS, Azure, GCP, cloud infrastructure
  if (lowerFieldName.includes('hosting')) {
    return `${domainFilter} "${appName}" infrastructure hosting AWS Azure GCP Google Cloud data center cloud hosting deployment ${!enforceOfficialDomain ? officialSourceHint : ''}`.trim();
  }
  
  // SSO Status - whether product supports SSO
  if (lowerFieldName.includes('sso')) {
    return `${domainFilter} "${appName}" SSO single sign-on support SAML OAuth authentication ${!enforceOfficialDomain ? officialSourceHint : ''}`.trim();
  }
  
  // Internal Comment (G2 URL) - search for G2 category page
  if (lowerFieldName.includes('internal comment')) {
    return `site:g2.com "${appName}" category software reviews`;
  }
  
  // Webpage URL / Website
  if (lowerFieldName.includes('webpage') || lowerFieldName.includes('website')) {
...
// Search for Pricing information (summary for Pricing Type field)
async function searchApplicationPricing(appName: string, vendorDomain: string | null, enforceOfficialDomain: boolean): Promise<PerplexitySearchResultWithQuality | null> {
  try {
    console.log(`[Pricing] Searching for ${appName} pricing info, enforceOfficial: ${enforceOfficialDomain}`);
    ...
    const response = await fetch('https://api.perplexity.ai/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `You are a pricing research assistant. Find pricing information for "${appName}".

SEARCH FOCUS:
1. Pricing plans (Free, Basic, Pro, Enterprise, etc.)
2. Monthly/annual pricing numbers
3. Free trial availability
4. Key pricing tiers and features

RESPONSE FORMAT:
- Summarize the pricing policy in ONE paragraph
- Include ALL pricing numbers (e.g., "$10/month", "$99/year")
- Maximum 200 characters
- If pricing is not publicly available, say "Contact for pricing" or "Not publicly available"
${enforceOfficialDomain && vendorDomain ? `\nONLY use information from ${vendorDomain}` : ''}`
          },
          {
            role: 'user',
            content: `What are the pricing plans and costs for "${appName}"? Summarize in one paragraph with all numbers.`
          }
        ],
        temperature: 0.1,
        top_p: 0.9,
    ...
    console.log(`[Pricing] Found ${citations.length} citations`);

    return {
      content: `[PRICING SEARCH]\n\n${content}`,
      urls: citations,
      isOfficialSource: enforceOfficialDomain
    };
  } catch (error) {
...
// Search for Pricing URL (official pricing page)
async function searchApplicationPricingUrl(appName: string, vendorDomain: string | null, enforceOfficialDomain: boolean): Promise<PerplexitySearchResultWithQuality | null> {
  try {
    console.log(`[Pricing URL] Searching for ${appName} pricing page, enforceOfficial: ${enforceOfficialDomain}`);
    ...
    const response = await fetch('https://api.perplexity.ai/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `You are a research assistant. Find the official pricing page URL for "${appName}".

SEARCH FOCUS:
1. Official pricing page URL
2. Plans and pricing page
3. Subscription/billing page

RESPONSE FORMAT:
- If pricing page exists: Provide the EXACT URL (e.g., https://example.com/pricing)
- If no public pricing page: State "Not available"
${enforceOfficialDomain && vendorDomain ? `\nONLY use URLs from ${vendorDomain}` : ''}`
          },
          {
            role: 'user',
            content: `Find the official pricing page URL for "${appName}". I need the direct link to their pricing page.`
          }
        ],
        temperature: 0.1,
        top_p: 0.9,
    ...
    if (!response.ok) {
      console.error(`[Pricing URL] API error:`, response.status);
      return null;
    }
    ...
    console.log(`[Pricing URL] Found ${citations.length} citations`);

    // Validate pricing URLs are accessible
    const validatedUrls = await validateUrls(citations, 2);

    return {
      content: validatedUrls.length > 0 
        ? `[PRICING URL SEARCH]\n\n${content}`
        : `[PRICING URL SEARCH - URLs could not be verified]\n\n${content}`,
      urls: validatedUrls,
      isOfficialSource: enforceOfficialDomain && validatedUrls.length > 0
    };
  } catch (error) {
    console.error('[Pricing URL] Error:', error);
    return null;
  }
}
...
- Description: Max 250 chars, what the application does
- SI ID: Product name with spaces removed (e.g., "SalesforceSalesCloud", "MicrosoftTeams")
- Webpage URL: Official product website URL
- Product Category: G2 category name. If product not on G2, use similar product's category
- Pricing Type: Summarize pricing policy in one paragraph, max 200 characters, include all pricing numbers
- Pricing URL: Official pricing page URL, or "Not available" if not found
- Hosting Description: Cloud infrastructure (e.g., "AWS", "Azure", "GCP", "AWS and Azure")
- SSO Status: "Yes" if SSO is supported, "No" if not, include supported providers if known
- Internal Comment: G2.com category page URL (e.g., https://www.g2.com/categories/crm)

CRITICAL SOURCING RULES:
1. ALL information must come from OFFICIAL sources (vendor website)
2. If information is NOT available on official sources, say "Not available"
3. Include the source URL in your reasoning
4. For G2 fields, use g2.com as the authoritative source
    ...
    if (componentName) {
      ...
      if (isApplicationWorkflow) {
        ...
        for (const field of fields) {
          ...
          // Check if this is a webpage URL field and productUrl was provided
          const isWebpageField = lowerFieldNameCheck.includes('webpage') || 
                                 lowerFieldNameCheck.includes('website') || 
                                 lowerFieldNameCheck.includes('homepage');
          
          // If webpage URL field and productUrl provided, use it directly (skip search)
          if (isWebpageField && productUrl) {
...
This is the official product URL provided by the user.`,
          ...
          if (needsSearch) {
            ...
            // Product Category field - search on G2.com
            if (lowerFieldName.includes('product category') || lowerFieldName.includes('category')) {
              console.log(`[Application] Product Category field - searching on G2.com`);
              const result = await searchApplicationG2Category(componentName, vendorDomainToUse);
              searchResults[field.fieldId] = result;
              // Cache for Internal Comment (G2 URL) field
              if (result) {
                searchResults['g2Cache'] = result;
              }
            }
            // Internal Comment field - G2 category page URL (reuse G2 search)
            else if (lowerFieldName.includes('internal comment')) {
              console.log(`[Application] Internal Comment field - reusing G2 Category search for URL`);
              // If G2 Category was already searched, reuse; otherwise search now
              if (!searchResults['g2Cache']) {
                const result = await searchApplicationG2Category(componentName, vendorDomainToUse);
                searchResults[field.fieldId] = result;
              } else {
                searchResults[field.fieldId] = searchResults['g2Cache'];
              }
            }
            // SSO Status field - search for SSO support
            else if (lowerFieldName.includes('sso')) {
              console.log(`[Application] SSO Status field - searching for SSO support`);
              const result = await searchApplicationSSOProviders(componentName, vendorDomainToUse, enforceOfficialDomain);
              searchResults[field.fieldId] = result;
            }
            // Pricing URL field - search for official pricing page
            else if (lowerFieldName.includes('pricing') && lowerFieldName.includes('url')) {
              console.log(`[Application] Pricing URL field - searching for pricing page`);
              const result = await searchApplicationPricingUrl(componentName, vendorDomainToUse, enforceOfficialDomain);
              searchResults[field.fieldId] = result;
            }
            // Pricing Type field - search for pricing summary
            else if (lowerFieldName.includes('pricing')) {
              console.log(`[Application] Pricing Type field - searching for pricing info`);
              const result = await searchApplicationPricing(componentName, vendorDomainToUse, enforceOfficialDomain);
              searchResults[field.fieldId] = result;
            }
            ...
            // Other fields - use standard search with domain enforcement
              ...
              if (enforceOfficialDomain && vendorDomainToUse) {
                ...
                if (result) {
                  ...
                  // If nothing found on official domain, say "Not available"
                  searchResults[field.fieldId] = {
                    content: `Not available - No information found on official source (${vendorDomainToUse}).`,
                    urls: []
                  };
                }
              } else {
